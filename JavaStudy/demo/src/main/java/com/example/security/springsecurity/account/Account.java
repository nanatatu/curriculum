...
@Entity						//Entityクラス
@Table(name="accounts")		//Entityに対するテーブル名を指定
public class Account implements UserDetails {

	private static final long serialVersionUID = 1L;

	//権限は一般ユーザ、マネージャ、システム管理者の３種類とする
	public enum Authority {ROLE_USER, ROLE_MANAGER, ROLE_ADMIN}		//enum = 複数の定数を格納
	...
	@Temporal(TemporalType.TIMESTAMP)								//@Temporal:日付と時刻
	private Date createdAt;

	// roleは複数管理できるように、Set<>で定義。
	@ElementCollection(fetch = FetchType.EAGER)						//基本型のコレクション型
